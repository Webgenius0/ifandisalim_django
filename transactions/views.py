...
def transaction_webhook(request):
    ...
    try:
        ...
        # print(data)
        extra_data = data['subscriber']['attributes']
        user_id = extra_data['db_user_id']['value']
        credit_amount = extra_data['credit_amount']['value']
        print(user_id)

        # credit_amount = data.get('extra_data',{}).get('credit_amount')
